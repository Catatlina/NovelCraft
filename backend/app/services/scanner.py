...
策略：httpx + BeautifulSoup 爬取各平台热榜，失败时返回空列表。
不依赖 AI Workbench 来源池，独立实现。

参考开源项目模式：
- novel-crawler 类项目: 多源并发爬取 → 标准化输出
- 每个源定义 URL + 选择器 + 解析函数
"""
import asyncio
import re
...
PLATFORM_SOURCES = {
    ...
    "番茄热门": {
        ...
        "selector": "div.rank-book-name",
        "type": "html",
        "region": "国内",
    },
    "晋江积分榜": {
        "url": "https://www.jjwxc.net/bookbase.php",
        "selector": "a[href*='onebook']",
        "type": "html",
        "region": "国内",
    },
    "纵横热榜": {
        "url": "https://www.zongheng.com/rank.html",
        "selector": "div.rank-list div.book-name a",
        "type": "html",
        "region": "国内",
    },
    "17K热榜": {
        "url": "https://www.17k.com/top/",
        "selector": "div.list div.book a",
        "type": "html",
        "region": "国内",
    },
    "书旗热榜": {
        "url": "https://www.shuqi.com/rank",
        "selector": "div.book-info h3 a",
        "type": "html",
        "region": "国内",
    },
    "飞卢热榜": {
        "url": "https://b.faloo.com/rank/",
        "selector": "div.rankList div.bookName a",
        "type": "html",
        "region": "国内",
    },
    "刺猬猫热榜": {
        "url": "https://www.ciweimao.com/book-rank",
        "selector": "div.rank-item a.title",
        "type": "html",
        "region": "国内",
    },
    "Webnovel Trending": {
        "url": "https://www.webnovel.com/ranking/power/1",
        "selector": "div.rank-item a.book-name, h3 a[href*='/book/']",
        "type": "html",
        "region": "海外",
    },
    "Royal Road Best": {
    ...
    "Wattpad Hot": {
        ...
        "selector": "a.story-title, div.story-info__title a",
        "type": "html",
        "region": "海外",
    },
    "ScribbleHub Latest": {
    ...
    "NovelUpdates Ranking": {
        ...
        "selector": "div.search_title a, div.wb-details a",
        "type": "html",
        "region": "海外",
    },
}
...
async def _scrape_platform(name: str, config: dict, timeout: int = 15) -> ScanResult:
    ...
    try:
        ...
        # Simple regex extraction based on common patterns
        # Extract title-like patterns from the HTML
        pattern = config.get("selector", "a[href]")
        # Fallback: extract any text that looks like a book title from ranking pages
        titles = re.findall(r'<a[^>]*>(.{2,30})</a>', html)
        seen = set()
        books = []
        for t in titles:
            t = re.sub(r'<[^>]+>', '', t).strip()
            if len(t) >= 2 and len(t) <= 30 and t not in seen:
                seen.add(t)
...
MOCK_SCAN_BOOKS: list[dict] = [
    ...
    {"title": "她 的名字叫红豆", "author": "晋江文学城作者", "platform": "晋江积分榜", "rank": 1, "hot_score": 88, "tags": ["言情", "现言", "虐恋"], "word_count": 800000, "summary": "一场误会，两次错过。他说红豆最相思。"},
